...
generateHPCWorkflow <- function(
  ...
  obs.meta.search <- subset(
    obs.meta, end.time >= search.start &
      start.time <= search.end + fcst.max.flt)
  if (operational) {
    obs.meta.test <- subset(
      obs.meta, end.time >= test.start &
        start.time <= test.end + fcst.max.flt)
  } else {
    obs.meta.test <- NULL
  }
...
distributeConfig <- function(
  test.start, test.end, search.start, search.end,
  fcst.max.flt, fcst.meta.test, fcst.meta.search,
  obs.meta.search, obs.meta.test,
  ...
  if (operational) {
    stopifnot(!is.null(obs.meta.test))
  }
    ...
    for (i.chunk.time in 1:length(chunks.time)) {
      ...
      rows.to.write <- c(
        ...
        addFiles(meta.table = obs.meta.search,
                 ...
                 station.end = station.chunk.end,
                 time.start = search.start,
                 time.end = search.end + fcst.max.flt))
      ...
      if (operational) {
        ...
        rows.to.write <- c(
          rows.to.write, 
          '# The following test forecast files are added into search',
          '# forecasts due to operational mode.', '',
          addFiles(
            ...
            time.end = time.chunk.end),
          addFiles(
            meta.table = obs.meta.test,
            file.type = 'search observation',
            station.start = station.chunk.start,
            station.end = station.chunk.end,
            time.start = time.chunk.start,
            time.end = time.chunk.end + fcst.max.flt))
      }
