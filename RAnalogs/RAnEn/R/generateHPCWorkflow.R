  ...
  for (i.chunk.station in 1:length(chunks.station)) {
    for (i.chunk.time in 1:length(chunks.time)) {
      ...
      rows.to.write <- c(
        ...
        ifelse(test = operational, no = '#',
               yes = c('#', '# Operational mode is used.', '#')),
      ...
      if (operational) {
        ...
        rows.to.write <- c(
          rows.to.write, 
          '# The following test forecast files are added in search forecasts due to operational mode',
          addFiles(
