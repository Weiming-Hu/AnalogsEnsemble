...
First, we set up the configuration. We generate AnEn for one day using about one year of historical data for all the grid points available.
...
par(mfrow = c(1, 3))
cols <- brewer.pal(11, 'Spectral')[11:1]
plot(rast.fcts, main = 'NAM Forecasts', col = cols)
map(col = 'grey', add = T); map('state', add = T)
plot(rast.anen, main = 'AnEn Averaged', col = cols)
map(col = 'grey', add = T); map('state', add = T)
plot(rast.anly, main = 'NAM Analysis', col = cols)
map(col = 'grey', add = T); map('state', add = T)
```
