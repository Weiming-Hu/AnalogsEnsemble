...
config$test_times_compare <- as.numeric(forecast.times[test.start:test.end])
...
AnEn.opr <- biasCorrection(AnEn, config, forecast.ID = 1, group.func = mean,
...
config$search_times_compare <- as.numeric(forecast.times[search.start:search.end])
...
rast.anen.opr.cor <- rasterize(xy[, c('x', 'y')], rast, mean.analogs.opr.cor[, 1, i.flt], fun = mean)
rast.anen.ind.cor <- rasterize(xy[, c('x', 'y')], rast, mean.analogs.ind.cor[, 1, i.flt], fun = mean)
