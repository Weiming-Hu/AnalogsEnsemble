...
# This script generates an R data file with analysis
library(ncdf4)
library(maps)

# Specify where the raw data are
data.folder <- '/media/wuh20/ExFat-Hu/Data/analysis_reformat'
...
for (i.file in 1:length(data.files)) {
  ...
  for (i in 1:length(indices)) {
    ...
    file.list[[i]] <- ncvar_get(nc, 'Data', start = c(1, indices[i], 1), count = c(num.pars, 1, num.times))
  }
...
analysis.times <- as.POSIXct(times, origin = '1970-01-01', tz = 'UTC')
num.times <- length(analysis.times)

# This variable is used to append data to the end of the data
recorded.days <- 0
analysis <- array(data = NA, dim = c(num.pars, num.stations, num.times))
for (i in 1:length(data.list)) {
  for (j in 1:length(data.list[[i]])) {
    analysis[, j, (recorded.days + 1):(recorded.days + dim(data.list[[i]][[j]])[2])] <- data.list[[i]][[j]]
  }
  recorded.days <- recorded.days + dim(data.list[[i]][[1]])[2]
}

# Make sure that there are same number of time values in analysis.times and analysis
