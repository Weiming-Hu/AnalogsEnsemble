...
void runAnalogGenerator(
        const vector<string> & files_test_forecasts,
    ...
    AnEnIO io("Read", files_test_forecasts[0], "Forecasts", verbose),
           io_out("Write", file_analogs, "Analogs", verbose);

    if (files_test_forecasts.size() ==1) {
        ...
        AnEnIO::combineForecastsArray(files_test_forecasts, test_forecasts,
                test_along, verbose, test_start, test_count);
    }
    ...
    if (files_observations.size() == 1) {
        ...
        AnEnIO::combineObservationsArray(files_observations, search_observations,
   ...
   handleError(io_out.writeAnalogs(
    ...
    cout << "-----------------------------------------------------" << endl
        << "User (CPU) Time for Analog Generator:" << endl
...
#if defined(_OPENMP)
    cout << "-----------------------------------------------------" << endl
        << "Real (Wall) Time for Analog Generator:" << endl
...
int main(int argc, char** argv) {
    ...
    // Required variables
    string file_analogs;
    vector<string> files_test_forecasts, files_observations, files_search_forecasts;
    size_t num_members = 0;

    // Optional variables
    int verbose = 0, time_match_mode = 1, max_sims_entries = -1,
        search_along = 0, obs_along = 0, test_along = 0;
    ...
    try {
        ...
        desc.add_options()
                ...
                ("test-forecast-nc", po::value< vector<string> >(&files_test_forecasts)->multitoken()->required(), "Set the input file(s) for test forecast NetCDF.")
                ("search-forecast-nc", po::value< vector<string> >(&files_search_forecasts)->multitoken()->required(), "Set input the file(s) for search forecast NetCDF.")
                ("observation-nc", po::value< vector<string> >(&files_observations)->multitoken()->required(), "Set the input file(s) for search observation NetCDF.")
    ...
    if (verbose >= 5) {
        cout << "Input parameters:" << endl
                << "files_test_forecasts: " << files_test_forecasts << endl
    ...
    try {
        runAnalogGenerator(
                files_test_forecasts, test_start, test_count,
                files_search_forecasts, search_start, search_count,
                files_observations, obs_start, obs_count,
