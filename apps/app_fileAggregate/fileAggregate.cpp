...
void runFileAggregate(const string & file_type, vector<string> & in_files, 
    ...
    if (partial_read) {
        if (file_type == "StandardDeviation") {
            cerr << BOLDRED << "Error: Currently, the partial read functionality is not supported for StandardDeviation."
                << RESET << endl;
            return;
        }
    }
       ...
       // Reshape data
        ...
        if (partial_read) {
            if (!(starts.size() == 5 * in_files.size() && counts.size() == 5 * in_files.size())) {
                cerr << BOLDRED << "Error: The count for start and count should be an integer multiplication of 3 for Similarity."
                    << RESET << endl;
                return;
            }
        }
        ...
        if (partial_read) {
            if (!(starts.size() == 5 * in_files.size() && counts.size() == 5 * in_files.size())) {
                cerr << BOLDRED << "Error: The count for start and count should be an integer multiplication of 3 for Analogs."
                    << RESET << endl;
                return;
            }
        }
    ...
    float duration_full = (float) (time_end - time_start) / CLOCKS_PER_SEC;

    cout << "-----------------------------------------------------" << endl
        << "User (CPU) Time for File Aggregate:" << endl
        << "Total: " << duration_full << " seconds" << endl
        << "-----------------------------------------------------" << endl;
#endif
