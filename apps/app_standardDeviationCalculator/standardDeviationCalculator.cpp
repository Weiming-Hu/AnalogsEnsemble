...
void runStandardDeviationCalculator(
    ...
    if (start.size() == 0 || count.size() == 0) {
        ...
        if (!(start.size() == 3 && count.size() == 3)) {
            cout << BOLDRED << "Error: start and count should both have 3 values."
                    << RESET << endl;
            return;
        }
    }
    ...
    for (size_t i = 0; i < in_files.size(); i++) {
        try {
            ...
            if (partial_read) {
                io.handleError(io.readForecasts(forecasts_vec[i], start, count));
            } else {
    ...
    if (verbose >= 3) cout << GREEN
            << "Combining forecasts along the time dimension..." << RESET << endl;
    ...
    if (verbose >= 3) cout << GREEN << "Compute standard deviation ..." << RESET << endl;
    AnEn anen(verbose);
    StandardDeviation sds;
    anen.handleError(anen.computeStandardDeviation(forecasts, sds));
    
    // Write standard deviation
    if (verbose >= 3) cout << GREEN << "Write the standard deviation ..." << RESET << endl;
    AnEnIO io("Write", out_file, "StandardDeviation", verbose);
    
    io.handleError(io.writeStandardDeviation(sds, forecasts));
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        desc.add_options()
                ...
                ("start", po::value< vector<size_t> >(&start)->multitoken(), "Set the start indices for each file. It specifies [parameter][station][FLT]")
                ("count", po::value< vector<size_t> >(&count)->multitoken(), "Set the count numbers for each file. It specifies [parameter][station][FLT]");
    ...
    try{
        runStandardDeviationCalculator(in_files, start, count, out_file, verbose);
    } catch (...) {
        handle_exception(current_exception());
        return 1;
    }
