...
Stations::getStationIndex(size_t station_ID) const {
    ...
    const multiIndexStations::index<By::ID>::type &
            stations_by_ID = get<By::ID>();
    ...
    if (it_ID != stations_by_ID.end()) {
        ...
        const multiIndexStations::index<By::insert>::type&
                stations_by_insert = get<By::insert>();
...
Stations::getStationsIndex(const vector<size_t> & stations_ID) const {
    ...
    auto & stations_by_insert = this->get<By::insert>();
...
Stations::getNearestStationsId(double main_station_x,
        double main_station_y, size_t num_nearest_stations,
    ...
    if (threshold == 0) {
        rtree.query(bgi::nearest(point(main_station_x, main_station_y),
                num_nearest_stations), back_inserter(results_points));
    } else {
        threshold *= threshold;
        rtree.query(bgi::nearest(point(main_station_x, main_station_y),
...
Stations::haveXY() const {
    ...
    const auto & stations_by_insert = get<By::insert>();
