...
AnEnIO::checkMode() const {
    ...
    if (mode_ != "Read" && mode_ != "Write") {
        if (verbose_ >= 1) {
            cerr << BOLDRED << "Error: Mode should be 'Read' or 'Write'."
                << endl;
        }
...
AnEnIO::checkFilePath() const {
    ...
    if (file_exists && mode_ == "Write") {
        if (add_) {
            ...
            if (verbose_ >= 1) {
                cerr << BOLDRED << "Error: File exists at "
                    << file_path_ << RESET << endl;
            }
        }
        ...
        if (verbose_ >= 1) {
            cerr << BOLDRED << "Error: File not found "
                << file_path_ << RESET << endl;
        }
        ...
        if (verbose_ >= 1) {
            cerr << BOLDRED << "Error: Unknown file extension "
                << file_path_ << RESET << endl;
        }
...
AnEnIO::checkFileType() const {
    ...
    if (mode_ == "Write") {
        ...
        if (file_type_ == "Observations") {
            ...
            if (verbose_ >= 1) {
                cerr << BOLDRED << "Error: Unknown file type "
        ...
        if (verbose_ >= 1) cerr << BOLDRED << "Error: Unknown mode."
            << RESET << endl;
        return (WRONG_MODE);
    }
}
...
AnEnIO::checkVariable(string var_name, bool optional) const {
    ...
    if (is_null) {
        if (optional) {
            ...
            if (verbose_ >= 1) {
                cerr << BOLDRED << "Error: Variable (" << var_name
                    << ") is missing in file (" << file_path_
                    << ")!" << RESET << endl;
...
AnEnIO::checkDim(string dim_name) const {
    ...
    if (is_null) {
        if (verbose_ >= 1) {
            cerr << BOLDRED << "Error: Dimension (" << dim_name
                << ") missing!" << RESET << endl;
        }
...
AnEnIO::readFLTs(anenTime::FLTs& flts) const {
    ...
    if (flts.size() != vec.size()) {
        if (verbose_ >= 1) {
            cerr << BOLDRED << "Error: The variable (FLTs)"
...
AnEnIO::readFLTs(anenTime::FLTs& flts,
    ...
    if (start + (count - 1) * stride >= dim_len) {
        if (verbose_ >= 1) cerr << BOLDRED
            << "Error: The FLT indices are not valid (start " << start
...
AnEnIO::readParameters(anenPar::Parameters & parameters) const {
    ...
    if (names.size() != dim_len) {
        if (verbose_ >= 1) {
            cerr << BOLDRED << "Error: There should be " << dim_len
...
AnEnIO::readTimes(anenTime::Times& times, const string & var_name) const {
    ...
    if (times.size() != vec.size()) {
        if (verbose_ >= 1) {
            cerr << BOLDRED << "Error: The variable (Times)"
...
AnEnIO::writeForecasts(const Forecasts & forecasts) const {
    ...
    if (!var_data.isNull()) {
        if (verbose_ >= 1) cerr << BOLDRED << "Error: Variable (Data)"
...
AnEnIO::writeFLTs(const anenTime::FLTs& flts, bool unlimited) const {
    ...
    if (!var.isNull()) {
        if (verbose_ >= 1) cerr << BOLDRED << "Error: Variable (FLTs)"
            << " exists in file (" << file_path_ << ")." << RESET << endl;
        nc.close();
        return (VARIABLE_EXISTS);
    }
    ...
    try {
        ...
        cerr << BOLDRED << "Error: Insufficient memory to write FLTs ("
...
AnEnIO::writeParameters(const anenPar::Parameters& parameters,
    ...
    try {
        ...
        cerr << BOLDRED << "Error: Insufficient memory to write Parameters ("
...
AnEnIO::writeTimes(const anenTime::Times& times, bool unlimited,
    ...
    if (!var.isNull()) {
        if (verbose_ >= 1) cerr << BOLDRED << "Error: Variable (" << var_name
            << ") exists in file (" << file_path_ << ")." << RESET << endl;
        nc.close();
        return (VARIABLE_EXISTS);
    }
    ...
    try {
        ...
        cerr << BOLDRED << "Error: Insufficient memory to write Times ("
            << times.size() << ")." << RESET << endl;
        nc.close();
        return (INSUFFICIENT_MEMORY);
    }
...
AnEnIO::readSimilarityMatrices(SimilarityMatrices & sims) {
    ...
    if (file_type_ != "Similarity") {
        if (verbose_ >= 1)
            cerr << BOLDRED << "Error: file type should be Similarity."
                << RESET << endl;
        return (WRONG_FILE_TYPE);
    }
...
AnEnIO::readSimilarityMatrices(SimilarityMatrices & sims,
    ...
    if (verbose_ >= 3) cout << "Reading partial similarity matrices ..." << endl;
    ...
    if (start.size() != 5) {
        if (verbose_ >= 1) cerr << BOLDRED
            << "Error: Start should have 5 indices for Similarity."
                << RESET << endl;
        return (WRONG_INDEX_SHAPE);
    }
    ...
    if (count.size() != 5) {
        if (verbose_ >= 1) cerr << BOLDRED
            << "Error: Count should have 5 indices for Similarity."
                << RESET << endl;
        return (WRONG_INDEX_SHAPE);
    }
    ...
    if (stride.size() != 5) {
        if (verbose_ >= 1) cerr << BOLDRED
            << "Error: Stride should have 5 indices for Similarity."
                << RESET << endl;
        return (WRONG_INDEX_SHAPE);
    }
...
AnEnIO::readAnalogs(Analogs & analogs) {
    ...
    if (file_type_ != "Analogs") {
        if (verbose_ >= 1)
            cerr << BOLDRED << "Error: file type should be Analogs."
                << RESET << endl;
        return (WRONG_FILE_TYPE);
    }
...
AnEnIO::readAnalogs(Analogs & analogs,
    ...
    if (verbose_ >= 3) cout << "Reading partial analogs ..." << endl;
    ...
    if (start.size() != 5) {
        if (verbose_ >= 1) cerr << BOLDRED
            << "Error: Start should have 5 indices for Analogs."
                << RESET << endl;
        return (WRONG_INDEX_SHAPE);
    }
    ...
    if (count.size() != 5) {
        if (verbose_ >= 1) cerr << BOLDRED
            << "Error: Count should have 5 indices for Analogs."
                << RESET << endl;
        return (WRONG_INDEX_SHAPE);
    }
    ...
    if (stride.size() != 5) {
        if (verbose_ >= 1) cerr << BOLDRED
            << "Error: Stride should have 5 indices for Analogs."
                << RESET << endl;
        return (WRONG_INDEX_SHAPE);
    }

    analogs.resize(boost::extents[count[0]][count[1]][count[2]][count[3]][count[4]]);

    // Read data
    if (verbose_ >= 3) cout << "Reading partial Analogs values from file ("
        << file_path_ << ") ..." << endl;
...
AnEnIO::readStandardDeviation(StandardDeviation & sds) {
    ...
    if (file_type_ != "StandardDeviation") {
        if (verbose_ >= 1)
            cerr << BOLDRED << "Error: file type should be StandardDeviation."
                << RESET << endl;
        return (WRONG_FILE_TYPE);
    }
...
AnEnIO::dumpVariable(string var_name, size_t start, size_t count) const {
    ...
    switch (var_type.getTypeClass()) {
        ...
        case NcType::nc_UINT64:
            ...
            break;
        default:
            if (verbose_ >= 1) cerr << BOLDRED << "Error: Variable ("
...
AnEnIO::combineParameters(
    ...
    if (!allow_duplicates) {
        if (parameters.size() != counter) {
            if (verbose >= 1) cerr << BOLDRED << "Error: Duplicates in parameters."
                << RESET << endl;
            return (ELEMENT_NOT_UNIQUE);
        }
    }
...
AnEnIO::combineStations(
    ...
    if (!allow_duplicates) {
        if (stations.size() != counter) {
            if (verbose >= 1) cerr << BOLDRED << "Error: Duplicates in stations."
                << RESET << endl;
            return (ELEMENT_NOT_UNIQUE);
        }
    }
...
AnEnIO::combineTimes(
    ...
    if (!allow_duplicates) {
        if (times.size() != counter) {
            if (verbose >= 1) cerr << BOLDRED << "Error: Duplicates in times."
                << RESET << endl;
            return (ELEMENT_NOT_UNIQUE);
        }
    }
...
AnEnIO::combineFLTs(
    ...
    if (!allow_duplicates) {
        if (flts.size() != counter) {
            if (verbose >= 1) cerr << BOLDRED << "Error: Duplicates in FLTs."
                << RESET << endl;
            return (ELEMENT_NOT_UNIQUE);
        }
    }
...
AnEnIO::combineForecastsArray(const vector<string> & in_files,
    ...
    for (size_t k = 0; k < in_files.size(); k++) {
        ...
        handleError(compareNewDimensions(same_dimensions, data_single.shape(), data.shape(), verbose));
...
AnEnIO::combineStandardDeviation(const vector<string> & in_files,
    ...
    for (const auto & file : in_files) {
        ...
        handleError(compareNewDimensions(same_dimensions, sds_single.shape(), sds.shape(), verbose));
...
AnEnIO::combineSimilarityMatrices(
    ...
    // Check whether partial reading is selected
    bool partial_read = false;
    if (starts.size() == 5 * in_files.size() && counts.size() == 5 * in_files.size()) {
        partial_read = true;
    }
    ...
    if (partial_read) {
        if (verbose >= 3) cout << "Processing partial meta information ..." << endl;
        handleError(io.readStations(stations, starts[0], counts[0]));
        handleError(io.readTimes(times, starts[1], counts[1]));
        handleError(io.readFLTs(flts, starts[2], counts[2]));

        size_t len = in_files.size();
        vector<size_t> starts_stations(len), counts_stations(len),
            starts_times(len), counts_times(len),
            starts_flts(len), counts_flts(len);

        for (size_t i = 0, j = 0; i < starts.size(); i+=5, j++) {
            starts_stations[j] = starts[i]; counts_stations[j] = counts[i];
            starts_times[j] = starts[i+1]; counts_times[j] = counts[i+1];
            starts_flts[j] = starts[i+2]; counts_flts[j] = counts[i+2];
        }

        if (along == 0) handleError(io.combineStations(in_files, "Similarity",
                    stations, verbose, "", "", starts_stations, counts_stations));
        else if (along == 1) handleError(io.combineTimes(in_files, "Similarity",
                    times, verbose, "Times", starts_times, counts_times));
        else if (along == 2) handleError(io.combineFLTs(in_files, "Similarity",
                    flts, verbose, starts_flts, counts_flts));
        else {
            cerr << BOLDRED << "Error: Appending along dimension " << along
                << " is not supported for partial reading." << RESET << endl;
        ...
        if (along == 4) {
            if (verbose >= 1) cerr << BOLDRED << "Error: Can not append along the dimension."
                << RESET << endl;
            return (ERROR_SETTING_VALUES);
        } else if (along == 3) {
            num_entries = 0;
    ...
    // Update dimensions
    if (verbose >= 3) cout << "Update dimensions of similarity ..." << endl;
    sims.setMaxEntries(num_entries);
    sims.resize(stations.size(), times.size(), flts.size());

    if (verbose >= 3) {
        if (partial_read) cout << "Copy partial similarity values into the new format ..." << endl;
        else cout << "Copy similarity values into the new format ..." << endl;
    }
    ...
    for (size_t k = 0; k < in_files.size(); k++) {
        ...
        if (verbose >= 4) cout << "Read file " << file << " ..." << endl;
        if (partial_read)  io_thread.readSimilarityMatrices(sims_single,
                {starts[k*5], starts[k*5+1], starts[k*5+2], starts[k*5+3], starts[k*5+4]},
                {counts[k*5], counts[k*5+1], counts[k*5+2], counts[k*5+3], counts[k*5+4]});
        else io_thread.readSimilarityMatrices(sims_single);

        handleError(compareNewDimensions(same_dimensions, sims_single.shape(), sims.shape(), verbose));
...
AnEnIO::combineAnalogs(const vector<string> & in_files,
    ...
    if (partial_read) {
        ...
        if (along == 0) handleError(io.combineStations(in_files, "Analogs",
                    stations, verbose, "", "", starts_stations, counts_stations));
        else if (along == 1) handleError(io.combineTimes(in_files, "Analogs",
                    times, verbose, "Times", starts_times, counts_times));
        else if (along == 2) handleError(io.combineFLTs(in_files, "Analogs",
                    flts, verbose, starts_flts, counts_flts));
        else {
            cerr << BOLDRED << "Error: Appending along dimension " << along
                << " is not supported for partial reading." << RESET << endl;
    ...
    if (verbose >= 3) {
        if (partial_read) cout << "Copy partial analogs values into the new format ..." << endl;
        else cout << "Copy analogs values into the new format ..." << endl;
    }
    ...
    for (size_t k = 0; k < in_files.size(); k++) {
        ...
        if (verbose >= 4) cout << "Read file " << file << " ..." << endl;
        if (partial_read) io_thread.readAnalogs(analogs_single,
                {starts[k*5], starts[k*5+1], starts[k*5+2], starts[k*5+3], starts[k*5+4]},
                {counts[k*5], counts[k*5+1], counts[k*5+2], counts[k*5+3], counts[k*5+4]});
        else io_thread.readAnalogs(analogs_single);

        handleError(compareNewDimensions(same_dimensions, analogs_single.shape(), analogs.shape(), verbose));
    ...
    if (var_dims.size() == 2) {
        if (var.getType() == NcType::nc_CHAR) {
            ...
            if (var_dims.at(0).getName() == "num_chars") {
                ...
                if (verbose_ >= 1) {
                    cerr << BOLDRED << "Error: The dimension (num_chars)"
...
#pragma omp parallel for default(none) schedule(static) \
    ...
    if (parameters.size() != vec_parameters.size()) {
        ...
        if (verbose_ >= 1) {
            cerr << BOLDRED << "Error: Parameters have duplicate! Total: "
...
#pragma omp parallel for default(none) schedule(static) \
    ...
    if (stations.size() != vec_stations.size()) {
        if (verbose_ >= 1) {
            cerr << BOLDRED << "Error: Stations have duplicates! Total: "
