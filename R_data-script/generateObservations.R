...
#
# This script is created for generating AnEn input observation file
...
# This variable is only used to determine how many times to create
#
dim.flts     <- 8
...
nc.dim.times <- ncdim_def( 
	"num_times", "", 1:(dim.times*dim.flts),
	create_dimvar = FALSE)
...
nc.var.data <- ncvar_def("Data", "", list(
	nc.dim.params, nc.dim.stations, nc.dim.times)
	,NA,prec="double")
...
nc.data.obs <- nc_create( output.name, list(
	nc.var.data, nc.var.params, nc.var.tz,
	nc.var.lon, nc.var.lat, nc.var.times,
	nc.var.stations))

ncvar_put( nc.data.obs, nc.var.data, data, start=c(1,1,1))
ncvar_put( nc.data.obs, nc.var.params, par.names)
ncvar_put( nc.data.obs, nc.var.tz, stations.tz)
ncvar_put( nc.data.obs, nc.var.lon, Xs)
ncvar_put( nc.data.obs, nc.var.lat, Ys)
